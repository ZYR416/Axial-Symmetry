...
class PPO:
    def __init__(self,
        ...
        self.critic = Critic(s_dim, a_dim, self.hidden)
    ...
    def store_transition(self, s, a, s_, r):
        # print(s, a, s_, r)
        self.memory_s.append(s.tolist())
        self.memory_a.append(a)
        self.memory_s_.append(s_.tolist())
    ...
    def _gae(self, s, r, s_):
        ...
        with torch.no_grad():
            v = self.critic(s[:, 0:2], s[:, 2:]).squeeze()        # [memory_len]
            v_ = self.critic(s_[:, 0:2], s_[:, 2:]).squeeze()      # [memory_len]
    ...
    def _discounted_r(self, r, s_):
        ...
        with torch.no_grad():
            ...
            v_ = self.critic(s_[:, 0:2], s_[:, 2:])    # [memory_len, 1]
    ...
    def _log_prob(self, s, a, old=False):
        ...
        if old:
            with torch.no_grad():
                mean, std = self.actor_old(s[:, 0:2], s[:, 2:])
        else:
            mean, std = self.actor(s[:, 0:2], s[:, 2:])
