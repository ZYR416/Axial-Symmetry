...
class DoubleCritic(nn.Module):
    def __init__(self, s_dim, a_dim, hidden):
        ...
        self.feature_h_2 = nn.Sequential(nn.Linear(2, hidden, bias=False),
                                         nn.Tanh(),
                                         nn.Linear(hidden, hidden, bias=False),
                                         nn.ReLU(),
                                         nn.Linear(hidden, 1, bias=False))

        self.feature_eva_2 = nn.Sequential(nn.Linear(5, hidden, bias=False),
                                           Abs(),
                                           nn.Linear(hidden, hidden, bias=False),
                                           Abs(),
                                           nn.Linear(hidden, 1, bias=False))
    ...
    def forward(self, s_h, s_ev, a):
        ...
        feature_h_2 = self.feature_h_2(s_h)
        feature_eva_2 = self.feature_eva_2(s_eva)
        q2 = torch.multiply(feature_h_2, feature_eva_2)
        return q1, q2
