...
def main(gamma):
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    agent = TD3(path, s_dim=6, gamma=0.98)
    agent.var = 0
    env = YawControlEnv()
    ...
    for net_num in range(200):
        ...
        agent.load_net(prefix1=str(2), prefix2=str(net_num))
        ...
        for ep_step in range(500):
            ...
            reward.append(r)
        rewards.append(np.sum(reward))
        agent.load_net(prefix1=str(6), prefix2=str(net_num))
        # agent.load_net(prefix1=str(i), prefix2=str(net_num))
        reward = []
        s = env.reset(target=[5, 0, 0])
        for ep_step in range(500):
            ...
            reward.append(r)
        rewards.append(np.sum(reward))
        agent.load_net(prefix1=str(9), prefix2=str(net_num))
        # agent.load_net(prefix1=str(i), prefix2=str(net_num))
        reward = []
        s = env.reset(target=[5, 0, 0])
        for ep_step in range(500):
