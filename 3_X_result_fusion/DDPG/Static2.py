...
def main():
    ...
    path += '/DDPG_'
    with open(path + '0.98/disr0.98.json', 'r') as f:
    ...
    for i in range(10):
        ...
        plt.plot(index, np.ones_like(index) * 5, label='0.98')
        plt.show()
    ...
    plt.plot(index, np.max(reward_store, axis=1), label='0.98')
    plt.plot(index, np.mean(reward_store, axis=1), label='0.98')
    plt.plot(index, np.min(reward_store, axis=1), label='0.98')
