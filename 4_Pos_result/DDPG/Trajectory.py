...
def main():
    ...
    diff = (targets[0:-1, 0:3] - targets[1:, 0:3]) / 0.01
    vel_bar = np.mean((diff[:, 0] ** 2 + diff[:, 1] ** 2 + diff[:, 2] ** 2) ** 0.5)

    error = np.mean(((px - tx) ** 2 + (py - ty) ** 2 + (pz - tz) ** 2) ** 0.5)
    print("position error:", error)
    error_psi = np.mean((yaw - tpsi) ** 2) ** 0.5
    print("ang error:", error_psi, error_psi / np.pi * 180)
    print(vel_bar)
    ...
    pos = np.array(pos)
    ang = np.array(ang)
    np.save(path + '/DDPG_' + 'Case2_' + name + '_pos.npy', pos)
    np.save(path + '/DDPG_' + 'Case2_' + name + '_ang.npy', ang)
