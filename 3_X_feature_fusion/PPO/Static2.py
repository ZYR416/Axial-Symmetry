...
def main():
    ...
    for i in range(10):
        ...
        plt.plot(index, np.ones_like(index) * 5, label='0.9')
        plt.show()
    ...
    plt.plot(index, np.max(reward_store, axis=1), label='0.9')
    plt.plot(index, np.mean(reward_store, axis=1), label='0.9')
    plt.plot(index, np.min(reward_store, axis=1), label='0.9')
